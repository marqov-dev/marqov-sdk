...
| Alice & Bob (local cat-qubit emulators — physical and logical/error-corrected tiers, no credentials) | Available: `pip install marqov[alicebob]`. Not combinable with `pytket`/`quantinuum`/`all` in the same environment — pins `qiskit<2.0`, which conflicts with `pytket-qiskit`'s current qiskit>=2.x requirement. |
